...
const Review = ({ comment, user }: { comment: UserReview; user: User }) => {
  return (
    <div className="bg-zinc-950/50 border group border-white/10 rounded-lg min-h-[80px] relative p-3 md:p-5 space-y-3 transition-all duration-200 ease-in-out">
      <div className="flex gap-1 ">
        ...
        <div className="flex flex-col gap-1">
          <Link href={`/novels/${comment.novelSlug}`} className="inline-flex gap-1 font-semibold text-sm text-zinc-300 hover:text-zinc-300/80 duration-200 transition-colors capitalize">
            {comment.novelTitle}
          </Link>
