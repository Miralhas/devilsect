...
export const DELETE = async (_: NextRequest, { params }: { params: Promise<{ novelSlug: string; id: number; chapterSlug: string }> }) => {
  try {
    const { id, novelSlug, chapterSlug } = await params;
    ...
    const url = `${env.APP_URL}/novels/${novelSlug}/chapters/${chapterSlug}/reviews/${id}`;
...
export const PUT = async (req: NextRequest, { params }: { params: Promise<{ novelSlug: string; id: number; chapterSlug: string }> }) => {
  try {
    const { id, novelSlug, chapterSlug } = await params;
    const session = (await getSession())?.value;
