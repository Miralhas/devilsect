      ...
      <div className="flex flex-1 flex-col gap-3.5 h-full">
        {res.results.slice(0, 4).map((novel, index) => (
          <div className="group w-full p-2 px-3 border hover:border-accent/60 hover:bg-primary/10 rounded-xl bg-muted-foreground/5 flex gap-3 items-center transition-all duration-[400ms] ease-in-out hover:translate-x-1 hover:-translate-y-1" key={index}>
            ...
            <div className="flex-1 gap-1.5 self-stretch flex flex-col py-1 group-hover:text-accent transition-colors duration-300 ease-in-out">
              <p className="font-bold line-clamp-1 text-sm text-[12px] md:text-[14px] overflow-hidden capitalize">{novel.title}</p>
              <div className="flex gap-3 items-center">
                <div className="text-xs font-bold flex gap-1 items-center text-muted-foreground group-hover:text-inherit">
                  <StarIcon className="size-3 text-yellow-500" fill="#D3AF37" />
                  <span className="text-xs text-[11px]">{novel.ratingValue ?? '4.74'}</span>
                </div>
                <div className="flex items-center gap-1 text-muted-foreground group-hover:text-inherit font-semibold">
                  <BookOpenText className="size-4" />
                  <p className="text-sm text-[12px] overflow-hidden line-clamp-1">{novel.chaptersCount} Chapters</p>
                </div>
              </div>
              <p className="text-sm text-[12px] text-muted-foreground font-semibold group-hover:text-inherit">{statusMap[novel.status]}</p>
            </div>
          </div>
        ))}
      </div>
