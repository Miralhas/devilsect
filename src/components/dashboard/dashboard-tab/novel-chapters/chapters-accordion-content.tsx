  ...
  return (
    <div className="grid md:grid-cols-2 pt-5 pb-3 px-7 gap-4 border-t">
      {chapters.data?.results.map(chapter => (
        <Link href={`/dashboard/${slug}/${chapter.slug}`} className="col-span-1 w-full border-b border-b-zinc-50/10 pb-3 transition-opacity duration-200 ease-in-out hover:opacity-70" key={chapter.id}>
